              ...
              <div className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex gap-0.5 shrink-0">
                    <button onClick={() => { setEditingProject(p); setIsProjectFormOpen(true); }} aria-label="Editar meta" className="p-2 text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><Pencil size={15} /></button>
                ...
                <div className="flex items-center justify-between mt-3">
                  <button onClick={() => setExpanded(isOpen ? null : p.id)} className="flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-indigo-600">
                    <ChevronDown size={14} className={`transition-transform ${isOpen ? 'rotate-180' : ''}`} />
                    {isOpen ? 'Ocultar tarefas' : `Ver tarefas (${total})`}
                  </button>
                  <button onClick={() => setTaskForm({ projectId: p.id, task: null })} className="flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-700">
                    <Plus size={14} /> Tarefa
                  </button>
                </div>
              </div>
