      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
          ...
          <div className="flex items-end gap-3">
            <div><p className="text-2xl font-bold text-indigo-600">{pomo.real}</p><p className="text-[11px] text-slate-400">reais 🍅</p></div>
            ...
            {focusByCategory.map(({ cat, minutes }) => (
              <div key={cat}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-bold text-indigo-600">{formatMinutes(minutes)}</span>
                </div>
                <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-indigo-500 transition-all duration-700" style={{ width: `${focusCatMax ? (minutes / focusCatMax) * 100 : 0}%` }} />
                </div>
              </div>
            ))}
          </div>
