...
const labelCls = 'block text-sm text-slate-600 mb-1.5';
const fieldCls = 'w-full px-4 py-3 rounded-xl border-0 bg-slate-50 text-[15px] text-slate-800 focus:ring-2 focus:ring-teal-300 outline-none transition placeholder:text-slate-400';
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm md:p-4">
      <div className="bg-white rounded-t-2xl md:rounded-2xl shadow-xl w-full max-w-md overflow-hidden max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <button
                ...
                className={`p-3 rounded-xl text-left transition ${urgent ? 'bg-teal-50' : 'bg-slate-50 hover:bg-slate-100'}`}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <Zap size={18} className={urgent ? 'text-teal-700' : 'text-slate-400'} />
                  <span className={`w-9 h-5 rounded-full p-0.5 transition-colors ${urgent ? 'bg-teal-700' : 'bg-slate-200'}`}>
                    <span className={`block w-4 h-4 bg-white rounded-full shadow transition-transform ${urgent ? 'translate-x-4' : ''}`} />
                  </span>
                </div>
                <p className={`text-sm font-semibold ${urgent ? 'text-teal-800' : 'text-slate-800'}`}>Urgente</p>
                <p className="text-[11px] text-slate-400 leading-snug">Tem prazo imediato</p>
              </button>
              <button
                type="button"
                onClick={() => setImportant(!important)}
                className={`p-3 rounded-xl text-left transition ${important ? 'bg-teal-50' : 'bg-slate-50 hover:bg-slate-100'}`}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <Star size={18} className={important ? 'text-teal-700' : 'text-slate-400'} />
                ...
                <p className={`text-sm font-semibold ${important ? 'text-teal-800' : 'text-slate-800'}`}>Importante</p>
                <p className="text-[11px] text-slate-400 leading-snug">Ajuda nos seus objetivos</p>
              </button>
            </div>
            <div className="mt-2 flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-50">
              <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: quadrant.color }} />
              <span className="text-xs font-semibold text-slate-800">{quadrant.label}</span>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {dateChips.map(opt => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-full text-sm font-medium transition active:scale-95 ${dueDate === opt.date ? 'bg-teal-50 text-teal-800' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  {opt.label}
                </button>
              ))}
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-sm text-slate-600">Subtarefas</label>
              {subtasks.length > 0 && <span className="text-[13px] text-slate-400 tabular-nums">{doneSubs}/{subtasks.length}</span>}
            </div>
            <div className="space-y-1.5">
              {subtasks.map(st => (
                <div key={st.id} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className={`flex-1 min-w-0 px-3.5 py-2.5 rounded-xl border-0 bg-slate-50 text-sm outline-none focus:ring-2 focus:ring-teal-300 ${st.done ? 'line-through text-slate-400' : 'text-slate-800'}`}
                  />
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 min-w-0 px-3.5 py-2.5 rounded-xl border-0 bg-slate-50 text-sm outline-none focus:ring-2 focus:ring-teal-300 placeholder:text-slate-400"
                />
                <button type="button" onClick={addSub} aria-label="Adicionar subtarefa" className="shrink-0 w-9 h-9 flex items-center justify-center text-teal-700 hover:bg-teal-50 rounded-xl active:scale-95 transition">
                  <Plus size={18} />
                </button>
              </div>
            </div>
          ...
          <div className="space-y-2 pt-1">
            ...
            {dueDate && title.trim() && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full py-2.5 rounded-xl bg-slate-100 text-slate-700 font-medium hover:text-[#4285F4] flex items-center justify-center gap-2 text-sm transition-colors"
              >
