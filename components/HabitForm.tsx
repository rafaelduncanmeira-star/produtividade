...
const labelCls = 'block text-[13px] font-medium text-slate-500 mb-2';
      ...
      <div className="bg-white rounded-t-2xl md:rounded-2xl shadow-xl w-full max-w-sm overflow-hidden max-h-[92vh] overflow-y-auto">
        <div className="px-5 py-4 border-b border-slate-100 flex justify-between items-center bg-white/95 backdrop-blur sticky top-0 z-10">
          <h3 className="font-semibold text-slate-800 text-[17px] flex items-center gap-2.5">
            <span className="w-7 h-7 rounded-lg bg-teal-50 text-teal-700 flex items-center justify-center"><Repeat size={15} strokeWidth={2.5} /></span>
            {initialHabit ? 'Editar Hábito' : 'Novo Hábito'}
          </h3>
          <button onClick={onClose} aria-label="Fechar" className="p-1.5 -mr-1.5 rounded-lg text-slate-400 hover:bg-slate-100 transition-colors"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-5 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
            ...
            <div className="grid grid-cols-6 gap-2">
              {HABIT_EMOJIS.map(e => (
                <button
                  ...
                  type="button"
                  onClick={() => setEmoji(e)}
                  className={`h-10 rounded-xl text-lg flex items-center justify-center transition-colors ${
                    emoji === e ? 'bg-teal-50 ring-2 ring-teal-300' : 'bg-slate-50 hover:bg-slate-100'
                  }`}
                >
                  {e}
                </button>
            ...
            <div className="flex gap-3">
              {HABIT_COLORS.map(c => (
                <button
                  ...
                  aria-label={`Cor ${c}`}
                  onClick={() => setColor(c)}
                  className={`w-8 h-8 rounded-full transition-transform ${color === c ? 'scale-110 ring-2 ring-offset-2 ring-teal-300' : 'hover:scale-105'}`}
          ...
          <div>
            ...
            {targetDays.length === 0 && (
              <p className="text-[13px] text-rose-500 mt-2">Escolha pelo menos um dia da semana.</p>
            )}
          </div>
